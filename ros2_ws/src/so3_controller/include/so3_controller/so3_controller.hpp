...
class SO3Controller {
 ...
 public:
  SO3Controller(const double& mass, const double& g) : mass_(mass), g_(g) {}
  ~SO3Controller() = default;

  // update state from sensors
  inline void setPos(const Eigen::Vector3d& pos) { pos_ = pos; }
  inline void setVel(const Eigen::Vector3d& vel) { vel_ = vel; }
  inline void setAcc(const Eigen::Vector3d& acc) { acc_ = acc; }

  inline Eigen::Vector3d getF() const { return force_; }
  inline Eigen::Quaterniond getQ() const { return quat_; }
  ...
  inline void calculateControl(const Eigen::Vector3d& des_pos,
    ...
    if (Eigen::Vector3d(0, 0, 1).dot(force_ / force_.norm()) < c) {
      const double nf = f.norm();
      const double A  = c * c * nf * nf - f(2) * f(2);
      const double B  = 2 * (c * c - 1) * f(2) * mass_ * g_;
      const double C  = (c * c - 1) * mass_ * mass_ * g_ * g_;
      const double s  = (-B + sqrt(B * B - 4 * A * C)) / (2 * A);
