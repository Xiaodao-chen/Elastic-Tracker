...
static const rclcpp::Logger kLogger = rclcpp::get_logger("mockamap");
...
Maps::pcl2ros()
{
  ...
  RCLCPP_INFO(kLogger, "finish: infill %lf%%",
...
Maps::perlin3D()
{
  ...
  RCLCPP_INFO(kLogger, "threshold: %lf", tmp);
  ...
  RCLCPP_INFO(kLogger, "the number of points before optimization is %d", info.cloud->width);
  info.cloud->points.resize(info.cloud->width * info.cloud->height);
  addGround();
  info.cloud->width = info.cloud->points.size();
  pcl2ros();
...
Maps::recursiveDivision(int xl, int xh, int yl, int yh, Eigen::MatrixXi& maze)
{
  ...
  { // the remaining area is larger than or equal to 5*5, need to add both x
    ...
    int  xm    = 0;
    int  ym    = 0;
    RCLCPP_INFO(kLogger, "entered 5*5 mode");
    while (valid == false)
    {
    ...
    RCLCPP_INFO(kLogger, "finished generating maze with width %d , height %d",
             xh - xl + 1,
             yh - yl + 1);
    std::cout << maze << std::endl;
    return;
  ...
  { // the case of 3*4+
    RCLCPP_INFO(kLogger, "entered 3*4+ mode");
    int doorcount = 0;
    int ym        = 0;
    for (int i = yl; i <= yh; i++)
    {
  ...
  { // the case of 4+*3
    RCLCPP_INFO(kLogger, "entered 4+*3 mode");
    int doorcount = 0;
    int xm        = 0;
    for (int i = xl; i <= xh; i++)
    {
...
Maps::recursizeDivisionMaze(Eigen::MatrixXi& maze)
{
  ...
  RCLCPP_INFO(kLogger, "debug %d %d %d %d", sx, sy, px, py);
  ...
  if ((sy - py - 3) > 0)
    ...
    y2 = py + 1;
  RCLCPP_INFO(kLogger, "%d %d %d %d", x1, x2, y1, y2);
...
Maps::Maze3DGen()
{
  ...
  RCLCPP_INFO(kLogger, "received parameters : numNodes: %d connectivity: "
  ...
  for (int i = 0; i < numNodes; i++)
  {
    ...
    RCLCPP_INFO(kLogger, "point: x: %f , y: %f , z: %f", rx, ry, rz);
