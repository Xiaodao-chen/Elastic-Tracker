...
EllipsoidArrayVisual::EllipsoidArrayVisual(Ogre::SceneManager *scene_manager, Ogre::SceneNode *parent_node)
...
void EllipsoidArrayVisual::setMessage(decomp_ros_msgs::msg::EllipsoidArray::ConstSharedPtr msg) {
  objs_.clear();
  if (!msg || msg->ellipsoids.empty()) {
    return;
  }
  ...
  for (auto &it : objs_) {
    it = std::make_unique<rviz_rendering::Shape>(
  ...
  for (const auto &it : msg->ellipsoids) {
    ...
    Ogre::Vector3 scale(
      2 * es.eigenvalues()[0], 2 * es.eigenvalues()[1], 2 * es.eigenvalues()[2]);
...
void EllipsoidArrayVisual::setFrameOrientation(const Ogre::Quaternion &orientation) {
