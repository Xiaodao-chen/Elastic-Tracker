...
TrajOpt::TrajOpt(const Params& params) {
  setParams(params);
}

void TrajOpt::setParams(const Params& p) {
  K_ = p.K;
  vmax_ = p.vmax;
  amax_ = p.amax;
  rhoT_ = p.rhoT;
  rhoP_ = p.rhoP;
  rhoTracking_ = p.rhoTracking;
  rhosVisibility_ = p.rhosVisibility;
  theta_clearance_ = p.theta_clearance;
  rhoV_ = p.rhoV;
  rhoA_ = p.rhoA;
  tracking_dur_ = p.tracking_dur;
  tracking_dist_ = p.tracking_dist;
  tracking_dt_ = p.tracking_dt;
  clearance_d_ = p.clearance_d;
  tolerance_d_ = p.tolerance_d;
}
...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  if (!extractVs(cfgHs_, cfgVs_)) {
    TRAJ_OPT_CORE_ERROR("extractVs fail!");
    return false;
  }
...
#ifndef NDEBUG
  {
    ...
    if (std::abs(z_start - z0) > 1e-6 || std::abs(z_end - z0) > 1e-6) {
      TRAJ_OPT_CORE_WARN_STREAM([&](auto& os) {
        os << "traj_opt: planar z constraint violated: z0=" << z0
           << " z_start=" << z_start
           << " z_end=" << z_end;
      });
    }
