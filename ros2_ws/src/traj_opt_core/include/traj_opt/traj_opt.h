...
#include "minco.hpp"
#include <traj_opt_core/logging.hpp>

namespace traj_opt {

struct Params {
  // # key points in time-integration penalty (see original implementation)
  int K = 8;
  ...
  double rhoV = 1.0;
  double rhoA = 1.0;
  double rhoTracking = 1.0;
  double rhosVisibility = 1.0;
  double clearance_d = 0.2;
  double tolerance_d = 0.2;
  double theta_clearance = 0.0;

  // tracking
  double tracking_dur = 2.0;
  double tracking_dist = 2.0;
  double tracking_dt = 0.05;
};
...
class TrajOpt {
 ...
 public:
  explicit TrajOpt(const Params& params = Params());
  ~TrajOpt() {}
