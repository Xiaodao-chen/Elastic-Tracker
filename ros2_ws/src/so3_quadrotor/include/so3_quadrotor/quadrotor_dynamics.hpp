...
class Quadrotor {
 ...
 public:
  const Config& config;
  const State& state;
  Quadrotor(const Config& conf) : config_(conf), config(config_), state(state_) {}
  ~Quadrotor() = default;
  ...
  inline State diff(const State& state) const {
    ...
    moments(2) =
      config_.km * (motor_rpm_sq(0) + motor_rpm_sq(1) - motor_rpm_sq(2) - motor_rpm_sq(3));

    double resistance = 0.1 * 3.14159265 * (config_.arm_length) * (config_.arm_length) *
                        state.v.norm() * state.v.norm();
    Eigen::Vector3d vnorm = state.v.norm() > 0 ? state.v.normalized() : state.v;

    state_dot.x = state.v;
    state_dot.v = -Eigen::Vector3d(0, 0, config_.g) + thrust * R.col(2) / config_.mass -
  ...
  // get control from cmd (ported as-is from ROS1)
  inline Control getControl(const Cmd& cmd) {
    ...
    float Om1 = state_.omega(0), Om2 = state_.omega(1), Om3 = state_.omega(2);
    ...
    float Psi = 0.5f * (3.0f - (Rd11 * R11 + Rd21 * R21 + Rd31 * R31 + Rd12 * R12 +
                                Rd22 * R22 + Rd32 * R32 + Rd13 * R13 + Rd23 * R23 +
                                Rd33 * R33));
    float force = 0;
    if (Psi < 1.0f)  // Position control stability guaranteed only when Psi < 1
      force = cmd.force[0] * R13 + cmd.force[1] * R23 + cmd.force[2] * R33;

    float eR1 = 0.5f * (R12 * Rd13 - R13 * Rd12 + R22 * Rd23 - R23 * Rd22 + R32 * Rd33 -
                        R33 * Rd32);
    float eR2 = 0.5f * (R13 * Rd11 - R11 * Rd13 - R21 * Rd23 + R23 * Rd21 - R31 * Rd33 +
                        R33 * Rd31);
    float eR3 = 0.5f * (R11 * Rd12 - R12 * Rd11 + R21 * Rd22 - R22 * Rd21 + R31 * Rd32 -
                        R32 * Rd31);
  ...
  inline void setPos(const Eigen::Vector3d& pos) { state_.x = pos; }
  inline void setYpr(const Eigen::Vector3d& ypr) { state_.R = uav_utils::ypr_to_R(ypr); }
  inline void setRpm(const Eigen::Vector4d& rpm) { state_.motor_rpm = rpm; }

  inline const Eigen::Vector3d& getPos() const { return state_.x; }
  inline Eigen::Vector3d getVel() const { return state_.v; }
  inline Eigen::Vector3d getAcc() const { return diff(state_).v; }
  inline Eigen::Vector3d getOmega() const { return state_.omega; }
  inline Eigen::Quaterniond getQuat() const { return Eigen::Quaterniond(state_.R); }
};
