...
TrajOpt::TrajOpt(rclcpp::Node::SharedPtr node) : node_(node) {
  ...
  // (some params may have been declared by TrackingFSM or Env already)
  ...
  declare_if_not("K", 8);
  declare_if_not("vmax", 1.0);
  declare_if_not("amax", 2.0);
  declare_if_not("rhoT", 100.0);
  declare_if_not("rhoP", 10000.0);
  declare_if_not("rhoTracking", 1000.0);
  declare_if_not("rhosVisibility", 10000.0);
  declare_if_not("theta_clearance", 0.8);
  declare_if_not("rhoV", 1000.0);
  declare_if_not("rhoA", 1000.0);
  declare_if_not("tracking_dur", 3.0);
  declare_if_not("tracking_dist", 2.5);
  declare_if_not("tracking_dt", 0.2);
  declare_if_not("clearance_d", 0.2);
  declare_if_not("tolerance_d", 0.3);
  ...
  node_->get_parameter("rhoTracking", rhoTracking_);
  node_->get_parameter("rhosVisibility", rhosVisibility_);
  node_->get_parameter("theta_clearance", theta_clearance_);
  ...
  node_->get_parameter("tracking_dt", tracking_dt_);
  node_->get_parameter("clearance_d", clearance_d_);
...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  if (!extractVs(cfgHs_, cfgVs_)) {
    RCLCPP_ERROR(node_->get_logger(), "extractVs fail!");
    return false;
  }
...
#ifndef NDEBUG
  {
    ...
    if (std::abs(z_start - z0) > 1e-6 || std::abs(z_end - z0) > 1e-6) {
      RCLCPP_WARN_STREAM(node_->get_logger(), "traj_opt: planar z constraint violated: z0=" << z0
                                                                    << " z_start=" << z_start
                                                                    << " z_end=" << z_end);
    }
  }
