...
class Env {
 ...
 public:
  Env(rclcpp::Node::SharedPtr node,
      std::shared_ptr<mapping::OccGridMap>& mapPtr) : mapPtr_(mapPtr), node_(node) {
    hPolyPub_ = node_->create_publisher<decomp_ros_msgs::msg::PolyhedronArray>("polyhedra", 1);
    // Use has_parameter check to avoid ParameterAlreadyDeclaredException
    // (these may have been declared by TrackingFSM::init already)
    if (!node_->has_parameter("tracking_dist")) {
      node_->declare_parameter<double>("tracking_dist", 0.0);
    }
    if (!node_->has_parameter("tolerance_d")) {
      node_->declare_parameter<double>("tolerance_d", 0.0);
    }
    if (!node_->has_parameter("theta_clearance")) {
      node_->declare_parameter<double>("theta_clearance", 0.0);
    }
    node_->get_parameter("tracking_dist", desired_dist_);
  ...
  inline void visCorridor(const vec_E<Polyhedron3D>& polyhedra) {
    ...
    poly_msg.header.stamp = node_->now();
  ...
  inline bool findVisiblePath(const Eigen::Vector3i& start_idx,
    ...
    double t_cost = (node_->now() - t_start_).seconds();
