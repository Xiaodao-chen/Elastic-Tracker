...
class Visualization {
 private:
  ...
  std::unordered_map<std::string, rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr> marker_pub_map_;
  std::unordered_map<std::string, rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr> path_pub_map_;
  std::unordered_map<std::string, rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr> pc_pub_map_;
  std::unordered_map<std::string, rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr> marker_array_pub_map_;
 ...
 public:
  Visualization(rclcpp::Node::SharedPtr node) : node_(node) {}
  ...
  void visualize_a_ball(const CENTER& c,
                        ...
                        const double a = 1) {
    auto got = marker_pub_map_.find(topic);
    if (got == marker_pub_map_.end()) {
      auto pub = node_->create_publisher<visualization_msgs::msg::Marker>(topic, 10);
      marker_pub_map_[topic] = pub;
    }
  ...
  void visualize_pointcloud(const PC& pc, const TOPIC& topic) {
    ...
    pc_pub_map_[topic]->publish(point_cloud_msg);
  }
  ...
  void visualize_arrow(const Eigen::Vector3d p0, const Eigen::Vector3d& p1, const TOPIC& topic, const Color& color = blue) {
    ...
    arrow_msg.header.stamp = node_->now();
    marker_pub_map_[topic]->publish(arrow_msg);
  }
  ...
  void visualize_balls(const BALLS& balls,
                       ...
                       const Color color = blue,
                       const double a = 0.2) {
    auto got = marker_array_pub_map_.find(topic);
    if (got == marker_array_pub_map_.end()) {
      auto pub = node_->create_publisher<visualization_msgs::msg::MarkerArray>(topic, 10);
      marker_array_pub_map_[topic] = pub;
    }
    visualization_msgs::msg::Marker marker;
    marker.header.frame_id = "world";
    marker.type = visualization_msgs::msg::Marker::SPHERE;
    ...
    marker_array_pub_map_[topic]->publish(marker_array);
  }
  ...
  void visualize_arrows(const ARROWS& arrows, const TOPIC& topic) {
    ...
    marker_array_pub_map_[topic]->publish(arrow_list_msg);
  }
  ...
  void visualize_traj_list(const TRAJLIST& traj_list, const TOPIC& topic) {
    ...
    marker_array_pub_map_[topic]->publish(path_list_msg);
  }
