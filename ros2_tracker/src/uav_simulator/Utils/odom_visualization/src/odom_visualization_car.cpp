...
void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
  ...
  // Car forces Z position to 0
  pose(2) = 0;
  // Car has different quaternion handling for 2D rotation
  ...
  // Car version uses transparent mesh color
  meshROS.color.a = 0;
  meshROS.color.r = 0;
  meshROS.color.g = 0;
  meshROS.color.b = 0;
  ...
  // Path
  static rclcpp::Time prevt = rclcpp::Time(msg->header.stamp.sec, msg->header.stamp.nanosec, RCL_ROS_TIME);
  ...
  static rclcpp::Time pt = rclcpp::Time(msg->header.stamp.sec, msg->header.stamp.nanosec, RCL_ROS_TIME);
  rclcpp::Time t = rclcpp::Time(msg->header.stamp.sec, msg->header.stamp.nanosec, RCL_ROS_TIME);
  if ((t - pt).seconds() > 0.5) {
  ...
  if (1) {
    ...
    // Car version uses hardcoded frame names (not drone_id based)
    std::string base_s = "/base";
    std::string laser_s = "/laser";
    std::string vision_s = "/vision";
    std::string height_s = "/height";

    // 发布 world -> base_s
    geometry_msgs::msg::TransformStamped transformStamped;
    transformStamped.header.stamp = msg->header.stamp;
    transformStamped.header.frame_id = "world";
    transformStamped.child_frame_id = base_s;
...
int main(int argc, char** argv) {
  ...
  auto node = rclcpp::Node::make_shared("odom_visualization_car");

  node->declare_parameter("mesh_resource", "package://odom_visualization/meshes/f250.dae");
